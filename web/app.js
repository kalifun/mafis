...
function applyTopologyByIndex(idx) {
    ...
    // Keep input-agents in sync so the burst kill% counter stays accurate
    const agentInput = document.getElementById('input-agents');
    if (agentInput) agentInput.value = agents;
    updateFlBurstAbs();
...
function simulateIn3D(idx) {
    ...
    setTimeout(() => {
        ...
        if (s.num_agents) { syncSliderDisplay('input-agents', 'val-agents', s.num_agents); updateFlBurstAbs(); }
        if (seed != null) {
