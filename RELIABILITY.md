...
| Time-weighted Throughput Error (TWTE) | yellow | Time-weighted absolute error of the throughput ratio post-fault (form adapted from the ITAE control penalty, Ogata 2010). Correct integration. Unbounded if baseline has near-zero ticks; downstream consumers must clamp or filter degenerate baselines. Data field name remains `itae` in code, CSVs, and JSON exports for backward compatibility with frozen runs. |
